...
THESPORTSDB_LEAGUES = {
    ...
    "USA": 4346,   # American Major League Soccer
}
THESPORTSDB_BASE = "https://www.thesportsdb.com/api/v1/json/123"

@st.cache_data(ttl=900, show_spinner=False)
def thesportsdb_fixtures(selected, days=45):
    """Fetch the next league events from TheSportsDB free V1 API."""
    rows = []
    now = pd.Timestamp.now()
    end = now + pd.Timedelta(days=days)

    for code in selected:
        league_id = THESPORTSDB_LEAGUES.get(code)
        ...
        try:
            ...
            r = requests.get(
                url, timeout=20,
                headers={
                    "User-Agent": "Mozilla/5.0 FootballValueLab/1.6.9",
            ...
            for ev in events:
                ...
                time_s = ev.get("strTime") or ev.get("strEventTime") or "00:00:00"
                dt = pd.to_datetime(
                    f"{date_s} {time_s}", errors="coerce"
                )
                if pd.isna(dt):
                    dt = pd.to_datetime(date_s, errors="coerce")
                if pd.isna(dt) or dt < now or dt > end:
                    continue
    ...
    cols = ["Date","LeagueCode","League","HomeTeam","AwayTeam","Source","EventID"]
    if not rows:
        return pd.DataFrame(columns=cols)

    return (
        pd.DataFrame(rows)
        .drop_duplicates(["LeagueCode","Date","HomeTeam","AwayTeam"])
        .sort_values("Date")
...
@st.cache_data(ttl=900, show_spinner=False)
def sofascore_fixtures(selected, days=45):
    """Fetch upcoming events from the current season of each tournament."""
    rows = []
    now = pd.Timestamp.now(tz="UTC")
    end = now + pd.Timedelta(days=days)

    for code in selected:
        tid = SOFASCORE_TOURNAMENTS.get(code)
        if tid is None:
            continue

        try:
            # Dynamically find the current season instead of hard-coding a season id.
            seasons_url = f"{SOFASCORE_API}/unique-tournament/{tid}/seasons"
            sr = requests.get(
                seasons_url, timeout=15,
                headers={"User-Agent": "Mozilla/5.0 FootballValueLab/1.6.9",
                         "Accept": "application/json"}
            )
            if sr.status_code != 200:
                continue

            seasons = sr.json().get("seasons", [])
            if not seasons:
                continue

            season = next(
                (s for s in seasons if str(s.get("year", "")).startswith("26/27")),
                seasons[0]
            )
            sid = season.get("id")
            if sid is None:
            ...
            # Fetch several pages of future events for the current season.
            for page in range(3):
                url = (
                    f"{SOFASCORE_API}/unique-tournament/{tid}/season/{sid}"
                    f"/events/next/{page}"
                )
                r = requests.get(
                    url, timeout=15,
                ...
                for ev in events:
                    ...
                    dt = pd.to_datetime(
                        ev.get("startTimestamp"), unit="s", utc=True, errors="coerce"
                    )
                    if pd.isna(dt) or dt < now or dt > end:
                        continue
...
def football_data_fixtures(days=45):
    """Retrieve upcoming fixtures from Football-Data.co.uk."""
    ...
    try:
        ...
        raw = pd.read_csv(BytesIO(r.content), encoding="latin1")
        ...
        for code, name in LEAGUES.items():
            x = raw[raw["Div"].astype(str).str.upper().eq(code)].copy()
            x = x[
                x["Date"].notna()
                & (x["Date"] >= now.normalize())
                & (x["Date"] <= horizon)
            ]
            if not x.empty:
                x["LeagueCode"] = code
                ...
                frames.append(
                    x[["Date", "LeagueCode", "League", "HomeTeam", "AwayTeam", "Source"]]
                )
...
def download_upcoming(selected):
    ...
    # Primary source: TheSportsDB free schedule API.
    ts = thesportsdb_fixtures(selected, days=45)
    for lc in selected:
        n = int((ts["LeagueCode"] == lc).sum()) if not ts.empty else 0
        status.append(f"{LEAGUES[lc]} TheSportsDB: {n}")
    if not ts.empty:
        frames.append(ts)

    # Secondary source: SofaScore.
    ss = sofascore_fixtures(selected, days=45)
    for lc in selected:
        n = int((ss["LeagueCode"] == lc).sum()) if not ss.empty else 0
        status.append(f"{LEAGUES[lc]} SofaScore: {n}")
    if not ss.empty:
        frames.append(ss)
    ...
    for lc in selected:
        ...
        status.append(f"{LEAGUES[lc]} ESPN: {len(f)}")
        if not f.empty:
            frames.append(f)

    # Last fallback: Football-Data.
    fd = football_data_fixtures(days=45)
    if not fd.empty:
        fd = fd[fd["LeagueCode"].isin(selected)]
        if not fd.empty:
            frames.append(fd)
            for lc in selected:
                status.append(
                    f"{LEAGUES[lc]} Football-Data: {int((fd['LeagueCode'] == lc).sum())}"
                )
    else:
        status.append("Football-Data: indisponible")
...
st.caption("V1.7.0 — 4 championnats • calendrier automatique • paper trading")
...
with st.sidebar:
    ...
    st.divider()
    st.caption("📅 Calendrier : TheSportsDB → SofaScore → ESPN → Football-Data")
...
try:
    ...
    if not predictions.empty:
        ...
        if "status" not in stored.columns:
    ...
    with tabs[3]:
        ...
        if not stored.empty:
            if "status" not in stored.columns:
                stored=stored.copy()
                stored["status"]="pending"
            settled=stored[stored["status"].astype(str).str.lower().eq("settled")]
        else:
