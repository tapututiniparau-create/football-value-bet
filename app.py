...
def sofascore_fixtures(selected, days=45):
    """Fetch future football events from SofaScore's public daily schedule."""
    rows = []
    now = pd.Timestamp.now(tz="UTC")
    end = now + pd.Timedelta(days=days)

    for offset in range(days + 1):
        d = now.normalize() + pd.Timedelta(days=offset)
        url = (
            "https://www.sofascore.com/api/v1/sport/football/"
            f"scheduled-events/{d.strftime('%Y-%m-%d')}"
        )
        try:
            r = requests.get(
                url, timeout=20,
                headers={
                    "User-Agent": "Mozilla/5.0 FootballValueLab/1.6.7",
            ...
            for ev in data.get("events", []):
                ...
                code = next(
                    (c for c in selected if SOFASCORE_TOURNAMENTS.get(c) == tid),
                    None
                )
                if code is None:
                    continue
    ...
    if not rows:
        return pd.DataFrame(
            columns=[
                "Date","LeagueCode","League","HomeTeam",
                "AwayTeam","Source","EventID"
            ]
        )
...
def football_data_fixtures(days=45):
    ...
    try:
        r = requests.get(
            url, timeout=25,
            headers={"User-Agent": "Mozilla/5.0 FootballValueLab/1.6.6"}
        )
...
st.caption("V1.6.7 — 4 championnats • calendrier automatique • paper trading")
