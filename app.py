...
ESPN_LEAGUES = {
    "F1": ("soccer/fra.1", "Ligue 1"),
    "E0": ("soccer/eng.1", "Premier League"),
    "SP1": ("soccer/esp.1", "La Liga"),
    "USA": ("soccer/usa.1", "MLS"),
}
FEATURES = [
...
def espn_events(league_code, days=45):
    ...
    rows=[]
    # ESPN scoreboard supports dates in 7-day windows; request weekly windows.
    for offset in range(0,days,7):
        d=today+pd.Timedelta(days=offset)
        dates=",".join(str(d+pd.Timedelta(days=i)) for i in range(min(7,days-offset)))
        url=f"https://site.api.espn.com/apis/site/v2/sports/{slug}/scoreboard?dates={dates.replace('-','')}"
        try:
            ...
            for ev in data.get("events",[]):
                ...
                rows.append({
                    "Date":dt.tz_convert(None) if pd.notna(dt) else pd.NaT,
...
def download_upcoming(selected):
    frames=[espn_events(lc) for lc in selected]
...
st.caption("V1.6.2 — 4 championnats • calendrier automatique • paper trading")
...
try:
    with st.spinner("Chargement de l'historique et du calendrier..."):
        ...
        fixtures=download_upcoming(selected)
    ...
    with tabs[0]:
        ...
        if predictions.empty: st.warning("Aucun match automatique récupéré. Vérifie les liens Flashscore dans la barre latérale.")
        else:
    ...
    with tabs[1]:
        ...
        if predictions.empty: st.info("Aucun match futur disponible.")
        else:
            d=predictions.copy()
