...
st.caption("V1.5 — prédictions, historique et suivi de performance")
...
def build_current_predictions(seasons_back):
    ...
    # Current season data contains fixtures already played + future fixtures.
    current_raw = download_data([current_year], DATA_DIR)

    hist_dataset = build_pre_match_dataset(raw_hist)
    model = train_model_all_history(hist_dataset)

    upcoming = current_raw[current_raw["FTHG"].isna() & current_raw["FTAG"].isna()].copy()
    played = current_raw[current_raw["FTHG"].notna() & current_raw["FTAG"].notna()].copy()

    predictions = predict_upcoming(model, hist_dataset, current_raw)
