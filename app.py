...
def download_upcoming(selected):
    ...
    # ESPN can be incomplete for schedules far enough in the future.
    for lc in selected:
    ...
    # Robust fallback: Football-Data publishes an updated fixture CSV.
    ...
    if not fd.empty:
        ...
        if not fd.empty:
            frames.append(fd)
            for lc in selected:
                n = int((fd["LeagueCode"] == lc).sum())
                status.append(f"{LEAGUES[lc]} Football-Data: {n}")
...
st.caption("V1.6.6 — 4 championnats • calendrier automatique • paper trading")
